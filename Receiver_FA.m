...
%------------------------FILTRO
w = 9600-2400;
filter = rectpuls((f-6000),w);
filter = filter + rectpuls((f+6000),w);
figure(1)
plot(f,filter);
%------------------------RUIDO AWGN
s_awgn = awgn(s_FSK,10000,'measured');
%------------------------RUIDO + SEÑAL (FREQ)
S_awgn = abs(fft(s_awgn,NFFT)/length(t2));
S_awgn = fftshift(S_awgn);
figure(2)
plot(f,S_awgn,'g')
hold on
plot(f,S_FSK,'r')
hold off
%------------------------RUIDO + SEÑAL (FREQ) - FILTRADO
figure(3)
S_filt = S_awgn.*filter;
plot(f,S_filt,'b')
figure(4)
s_rec = ifftshift(S_filt);
plot(f,s_rec)
figure(5)
s_rec = real(ifft(s_rec));
plot(t1,s_rec)
pos1 = find(s_rec>(mean(s_rec) + 3.5*std(s_rec)),length(s_rec));
pos2 = find(s_rec<(mean(s_rec) - 3.5*std(s_rec)),length(s_rec));
pos = [pos1 pos2];
for i=1:length(pos)
    s_rec(pos(i)) = mean(s_rec);
end
figure(6)
plot(t1,s_rec)
%% RECEPTOR
