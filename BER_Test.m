...
i_bits = 1e8;            %Bits de informacion
if (


for bit_now = 1:t_bits
...
for ciclos = 1:2
    ...
    %% CONTEO DE ERRORES
    comp = xor(logical(y_decod),logical(vector));
    errors = find(comp==1,length(comp));
    errors = length(errors);
    BER = errors/t_bits;
    vector_error = [vector_error errors];
    end
    check_encoder = 1;
end